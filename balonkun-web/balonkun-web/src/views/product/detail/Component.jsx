...
const ZoomPanel = ({ data }) => {
	...
	return (
		<div 
			className="hidden lg:block absolute left-[102%] top-0 w-[120%] h-full bg-white z-[999] border border-slate-200 shadow-2xl overflow-hidden rounded-sm"
			...
			<Container className="pt-6 max-w-[1440px]">
				<div className="flex flex-col lg:flex-row gap-4 relative items-start bg-white p-4 sm:p-6 shadow-sm border border-slate-200 rounded-sm">
					...
					<div className={`w-full lg:w-[40%] flex flex-col gap-4 lg:sticky lg:top-32 ${zoomData.show ? 'z-[50]' : 'z-10'}`}>
						<div className="detail-view-wrapper border border-slate-200 p-2 bg-white relative aspect-square group">
							<Swiper
								spaceBetween={10}
								navigation={{
								...
								loop={true}
								className="mySwiper h-full overflow-hidden rounded-sm"
							>
						...
						<div className="thumbnail-wrapper w-full overflow-hidden">
							...
							>
								{pictures.map((picture, index) => (
									<SwiperSlide key={index} className="cursor-pointer border border-slate-200 rounded-sm overflow-hidden aspect-square transition-all hover:border-[#ffb400]">
					...
					<div className="w-full lg:w-[60%] flex flex-col lg:pl-4">
						...
						<div className="flex flex-col gap-3 mt-8">
							<div className="flex items-center gap-3 w-full">
								<button
									className="flex-1 py-4 bg-[#fb641b] text-white font-bold text-[18px] shadow-lg shadow-[#fb641b]/20 hover:bg-[#fb641b]/90 uppercase flex items-center justify-center gap-2 rounded-sm transition-all"
								...
								<button
									className="flex-1 py-4 bg-[#ff9f00] text-white font-bold text-[16px] shadow-sm hover:bg-[#ff9f00]/90 uppercase flex items-center justify-center gap-2 rounded-sm"
									onClick={() => {
							...
							<button
								className="w-full py-3 bg-white text-slate-700 font-bold border border-slate-300 hover:bg-slate-50 transition-colors uppercase flex items-center justify-center gap-2 rounded-sm shadow-sm"
				...
				<div className="mt-8 flex flex-col gap-8">
					...
					<div className="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-slate-100 overflow-hidden">
						<RelatedProducts
							product={product}
