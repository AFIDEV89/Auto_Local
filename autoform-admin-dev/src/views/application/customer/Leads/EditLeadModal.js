...
const EditLeadModal = ({
    ...
    const handleSave = async () => {
        if (formValues.contact_no === undefined || formValues.contact_no === "") {
        ...
        try {
            ...
            if (isEditing) {
                const response = await API.put("/lead/admin/" + id, { ...formValues, id });
                if (response.status >= 200 && response.status < 300)  {
                    successAlert("Lead Edited Successfully");
                    closeModal();
                }
            } else {
                const response = await API.post("/lead", formValues);
                    if (response.status >= 200 && response.status < 300) {
            ...
            console.log(error)
            errorAlert("Something went wrong while adding lead");
            closeModal();
        }
    }
    return (
        ...
        >
            <DialogTitle>
                {isEditing ? "Edit" : "Add"} Lead Details
            </DialogTitle>
