...
const SubCategoryList = () => {
  ...
  const [openAddSubCategory, setOpenAddSubCategory] = React.useState(false);
  const [editSubCategoryData, setEditSubCategoryData] = React.useState({});
  const [categoryListData, setCategoryListData] = React.useState([]);

  const handleSubCategoryList = async (page) => {
    try {
      const response = await API.get(
        `/subcategory/get-list?page=${page}&limit=${rowsPerPage}`
      );
      ...
      apiErrorHandler(
        error,
        "Something went wrong while getting the SubCategory List"
      );
    }
  };

  React.useEffect(() => {
    setRows(subCategoryList);
  }, [subCategoryList]);
    ...
    if (newString) {
      const newRows =
        ...
        rows?.filter((row) => {
          ...
          // Check category name from categoryListData
          const categoryName = categoryListData?.find((e) => e.id === row.category_id)?.name || "";
          if (categoryName.toLowerCase().includes(newString.toString().toLowerCase())) {
            containsQuery = true;
          }
  ...
  const handleDelete = async () => {
    try {
      const response = await API.delete(`/subcategory/delete/${deleteId}`);
  ...
  return (
    <MainCard
      title="Product SubCategory List"
      content={false}
      secondary={!isUserModerator && (
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              <CsvToolbar entityName="subcategories" onImportComplete={reloadApi} />
              <Button
                  variant="text"
                  onClick={() => {
    ...
    >
      <CardContent align="right">
        <TextField
          ...
          placeholder="Search Product SubCategory "
          value={search}
          size="small"
        />
      </CardContent>
              ...
              .map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {categoryListData &&
                        categoryListData?.find((e) => e.id === row.category_id)
                        ?.name}
                    </TableCell>
                    <TableCell
                      component="th"
