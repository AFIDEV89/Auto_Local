...
const CreateMaterial = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await API.post("/material/create", payload);

      if (response && response.data && response.data.data) {
        handleCloseModal();
        reloadApi();
        setTimeout(() => {
          successAlert("Material added successfully.");
        }, 200);
      } else {
      ...
      apiErrorHandler(error, "Something went wrong while adding the Material.");
    }
  };
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const response = await API.put(`/material/update/${editMaterialData.id}`, payload);
      if (
        response &&
        response.data &&
        ...
        reloadApi();
        setTimeout(() => {
          successAlert("Material updated successfully.");
        }, 200);
      } else {
        if (response.data) {
          errorAlert(response.data.message);
  ...
  const isEdit = editMaterialData && editMaterialData.id;
  useEffect(() => {
    if (editMaterialData.id) {
      setName(editMaterialData.name);
      setPicUrl(editMaterialData.image);
    }
  }, [editMaterialData, editMaterialData.id]);
